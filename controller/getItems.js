...
const SetItemsTag = require("./setItemsTag");

async function GetItems(userIds, tag, next = null, collectedData = []) {
  ...
  var request2 = https.request({
    ...
    path: "/v2024.2.LTS/itembank/items",
    method: "POST",
  ...
  try {
    ...
    if (success && result?.data?.length > 0) {
      console.log(`Now Setting Items Tags ${tag}`);
      const itemsArray = result?.data?.map((val) => ({
      ...
      try {
        await SetItemsTag(itemsArray, tag);
      } catch (error) {
        console.log(`failed to set this items tag ${tag}`, error);
      }
    }
    ...
    if (success && nextValue) {
      return GetItems(userIds, tag, nextValue, collectedData);
    }
